...
Object.assign(globalThis, {
    ...
    TAGS_ALL: "tags-all",
    TRACKERS_ALL: "trackers-all",
    TRACKERS_ANNOUNCE_ERROR: "trackers-announce-error",
    TRACKERS_ERROR: "trackers-error",
    TRACKERS_TRACKERLESS: "trackers-trackerless",
    TRACKERS_WARNING: "trackers-warning",
    selectedCategory: "categories-all",
    selectedStatus: "all",
    selectedTag: "tags-all",
    selectedTracker: "trackers-all",
    trackerMap: new Map(),
    trackerMapRevision: 0,
});
...
const addTorrentFilterControls = () => {
    document.body.insertAdjacentHTML("beforeend", `
        <input id="torrentsFilterInput">
        <input id="torrentsFilterRegexBox" type="checkbox">
        <select id="torrentsFilterSelect">
            <option value="name">Name</option>
            <option value="save_path">Save Path</option>
        </select>
    `);
};

const addTorrent = (table, rowId, name, savePath) => {
    table.updateRowData({
        rowId: rowId,
        name: name,
        save_path: savePath,
...
test("torrent filtering reacts to filter-by changes without row updates", () => {
    const table = createTableFixture(DynamicTable.TorrentsTable);
    addTorrentFilterControls();
    addTorrent(table, "name-match", "linux image", "/downloads/other");
    addTorrent(table, "path-match", "other image", "/downloads/linux");

    document.getElementById("torrentsFilterInput").value = "linux";
    document.getElementById("torrentsFilterSelect").value = "name";
    expect(rowIds(table.getFilteredAndSortedRows())).toStrictEqual(["name-match"]);

    document.getElementById("torrentsFilterSelect").value = "save_path";
    expect(rowIds(table.getFilteredAndSortedRows())).toStrictEqual(["path-match"]);
});

test("torrent filtering reacts to tracker membership revisions without row updates", () => {
    const table = createTableFixture(DynamicTable.TorrentsTable);
    addTorrentFilterControls();
    addTorrent(table, "first", "first", "/downloads/first");
    addTorrent(table, "second", "second", "/downloads/second");

    selectedTracker = "tracker.example";
    trackerMap.set(selectedTracker, new Map([
        ["https://tracker.example/announce", new Set(["first"])]
    ]));
    ++trackerMapRevision;
    expect(rowIds(table.getFilteredAndSortedRows())).toStrictEqual(["first"]);

    trackerMap.get(selectedTracker).set("https://tracker.example/announce", new Set(["second"]));
    ++trackerMapRevision;
    expect(rowIds(table.getFilteredAndSortedRows())).toStrictEqual(["second"]);
});
...
test("virtual scroll rerendering is throttled through requestAnimationFrame", () => {
    ...
    table.dynamicTableDiv.scrollTop = 1;
